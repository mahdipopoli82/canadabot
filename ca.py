...
def main_buttons(user_id=None):
    ...
    return [
        [Button.inline("🔢 Get Code", b"get_code")],
        [Button.inline("➡️ Next", b"get_next")],
        [
            Button.inline("📊 Line", b"line"),
            Button.inline("♻️ Reset", b"reset"),
...
# دکمه Reset (شیشه‌ای)
# =========================
@client.on(events.CallbackQuery(data=b"reset"))
async def reset_callback(event):
    user_id = event.sender_id
    if not is_allowed(user_id):
        await event.answer("❌ Access Denied", alert=True)
        return
