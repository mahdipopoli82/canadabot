...
def main_buttons(user_id=None):
    ...
    return [
        ...
        [
            Button.inline(f"⬅️ Prev", b"get_prev"),
...
# Previous Number
# =========================
@client.on(events.CallbackQuery(data=b"get_prev"))
async def get_prev(event):
    user_id = event.sender_id
    if not is_allowed(user_id):
        await event.answer("❌ Access Denied", alert=True)
        return
    ...
    # Go back: index points to next-to-show, subtract 2 to get previous
    new_index = user_index[user_id] - 2
    if new_index < 0:
        new_index = 0

    user_index[user_id] = new_index
